...
class handler extends CI_Controller {
    ...
    public function create_customer()
    {
        ...
        // Create a Customer
        $customer = \Stripe\Customer::create(array("source" => $token, "description" => "Charles Brian"));
    ...
    // After we create a customer, we want to store their information in the database.
    public function charge($user_id, $price)
    {
      // We'll have to pass stripe a customer id that we retrieve from the database using a user_id.
        $this->load->model('user');
        $customer_id = $this->user->get_customer_id($user_id);

        \Stripe\Charge::create(array("amount" => $price, "currency" => "usd", "customer" => $customer_id));
    }
}
?>
